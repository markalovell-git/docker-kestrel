...
async def compose_drift(args: ComposeDriftInput) -> dict:
    ...
    for service_name, service_def in services.items():
        declared_image = service_def.get("image", f"{service_name}:latest")
        declared_env = _normalize_env(service_def.get("environment", {}))
        declared_ports = _normalize_ports(service_def.get("ports", []))
        ...
        for container_port, bindings in actual_ports.items():
            if bindings:
                for b in bindings:
                    actual_port_strs.append(f"{b.get('HostPort', '')}:{container_port.split('/')[0]}")
